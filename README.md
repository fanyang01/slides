# Slides

## Machine Learning

+ [Likelihood-based Models for Out-of-Distribution Detection](pdf/DGM-for-OOD.pdf)
+ [Deep Generative Models](pdf/Deep-Generative-Models.pdf)
+ [Probabilistic Models for Sequential Data](pdf/Deep-SSM.pdf)
+ [Variational Inference](pdf/Variational-Inference.pdf)
+ [Graph Neural Networks](pdf/Graph-Neural-Networks.pdf)

## Database

+ [Data Stream Synopses](pdf/Sketch.pdf)
+ [Lightweight Indexes for Massive Datasets](pdf/Bitmap-Index.pdf)
+ [Approximate Query Processing (中文)](pdf/AQP.pdf)
+ [Non-Volatile Memory in DBMS](pdf/Non-Volatile-Memory-in-DBMS.pdf)
+ [Answer Queries using Views (中文)](pdf/Answer-Queries-using-Views.pdf)
+ [Automatic Physical DB Design (中文)](pdf/Automatic-Physical-DB-Design-Chinese.pdf)
+ [Sampling over Joins](pdf/Sampling-over-Joins.pdf)


## Misc

+ [Causality Analysis for Attack Investigation](pdf/Causality-Analysis-for-Attack-Investigation.pdf)
+ [Consistent Hashing](pdf/Consistent-Hashing.pdf)